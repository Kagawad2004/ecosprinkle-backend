 ...
 * 
 * Uses shared MQTT client from mqttClient service for reliable message delivery.
 */
...
class WatchdogService {
  constructor() {
    ...
    console.log('🐕 WatchdogService initialized (using shared MQTT client)');
  }
   ...
   */
  _isMqttReady() {
    return mqttClientService.isClientConnected();
  }
  ...
  sendWiFiResetCommand(deviceId) {
    ...
    console.log(`🐕 Sending WiFi reset to ${deviceId}...`);
    
    // Use shared MQTT client (handles queueing automatically)
    return mqttClientService.publish(topic, payload, { qos: 1 })
      .then(() => {
        console.log(`🐕✅ WiFi reset command sent to ${deviceId}`);
      })
      .catch((error) => {
        console.error(`🐕❌ Failed to send WiFi reset to ${deviceId}:`, error.message);
        throw error;
      });
  }
  ...
  sendRegistrationConfirmation(deviceId) {
    ...
    console.log(`🐕 Sending registration confirmation to ${deviceId}...`);
    ...
    return mqttClientService.publish(topic, payload, { qos: 1 })
      ...
      .catch((error) => {
        console.error(`🐕❌ Failed to send registration confirmation to ${deviceId}:`, error.message);
